...
//	   "ＭＳ Ｐゴシック"     // フォントのタイプフェイス名の指定。これは見たまんま。
	   "Ariel"     // フォントのタイプフェイス名の指定。これは見たまんま。
   );

	if(font > 0) {
	    HANDLE pFont = SelectObject(gr.hdcMem,font);        // フォントを設定。
		/* GDI で文字を描画 */
		SetTextColor(gr.hdcMem, color);
		...
		SelectObject(gr.hdcMem,pFont);      // フォントを元に戻す。
		DeleteObject(font); 				/* フォント開放 */
	}
}
